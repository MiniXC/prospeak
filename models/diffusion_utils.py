...
class DiffusionSampler():
    ...
    def __call__(self, size, condition, speaker, device, N=4, return_sequence=False, seed=None):
    ...
    def sampling_given_noise_schedule(
        ...
        x = torch.normal(0, 1, size=size).to(device=device)
        if return_sequence:
            x_ = copy.deepcopy(x)
            xs = [x_]
        with torch.no_grad():
            for n in range(N - 1, -1, -1):
                diffusion_steps = (steps_infer[n] * torch.ones((size[0], 1))).to(device=device)
                diffusion_steps = diffusion_steps.unsqueeze(1)
                epsilon_theta = self.model(x, condition, speaker, diffusion_steps)
                if ddim:
                    ...
                    if n > 0:
                        x = x + sigma_infer[n] * torch.normal(0, 1, size=size).to(device=device)
                if return_sequence:
                    x_ = copy.deepcopy(x)
                    xs.append(x_)
        if return_sequence:
